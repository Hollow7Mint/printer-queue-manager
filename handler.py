"""Printer Queue Manager — Job handler layer."""
...
class PrinterHandler:
    """Job handler for the Printer Queue Manager application."""
    ...
    def __init__(
        ...
        self._store = store
        self._cfg   = config or {}
        self._document_name = self._cfg.get("document_name", None)
        logger.debug("%s initialised", self.__class__.__name__)

    def pause_job(
        self, document_name: Any, status: Any, **extra: Any
    ) -> Dict[str, Any]:
    ...
    def retry_job(self, record_id: str) -> bool:
        """Remove a Job; returns True on success."""
        if self._store.get(record_id) is None:
            return False
        self._store.delete(record_id)
        logger.info("retry_job: removed %s", record_id)
        return True
